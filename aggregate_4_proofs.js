...
async function prove_UltraHonk_CLI() {
  ...
  // === Generate 2 aggregation proofs to aggregate all 4 Semaphore proofs ===
  // === 1 ===
  ...
  // The vk is the same for both aggregation proofs
  runBB([
    "write_vk", "-v",
    "--scheme", "ultra_honk",
    "--output_format", "bytes_and_fields",
  ...
  */
  ...
  const vkFields_final = JSON.parse(readFileSync(`${outJoin1}/vk_fields.json`));

  // Witness generation with noir_js works fine
  const CIRCUIT_ROOT = new Noir(CIRCUITS.join_agg_proofs);
  const { witness: witness_root } = await CIRCUIT_ROOT.execute({
    agg1_verification_key: vkFields_final,
    agg1_proof: proofFields_join1,
    agg1_key_hash: vkHash,
    agg2_verification_key: vkFields_final,
